...
description: Reference knowledge for application security and SecOps — OWASP Top 10 (Web 2021, API 2023, Cloud-Native), security tooling (SAST, DAST, SCA, container/IaC scanning, secrets scanning, code review automation), secure coding practices (input validation, injection/XSS/CSRF prevention, authentication, authorization, cryptography, secrets management, API security, dependency management, security logging), CI/CD pipeline and supply chain security, incident response, and the security-focused code review checklist. Load this BEFORE performing a security review, vulnerability analysis, threat modeling, hardening CI/CD, or writing security-sensitive code.
---

# SecOps & Application Security Reference
...
- [/threat-model](../threat-model/SKILL.md) — STRIDE threat model for a feature, component, or system boundary (mandatory for new attack surface)
- [/oauth-threat-model](../oauth-threat-model/SKILL.md) — focused threat model for OAuth2/OIDC flows (PKCE, token endpoints, redirect URIs, token theft)
- [/audit-jwt-config](../audit-jwt-config/SKILL.md) — audit JWT implementations for algorithm confusion, claims gaps, and token lifecycle weaknesses
- [/dependency-review](../dependency-review/SKILL.md) — review dependency upgrades for breaking changes, CVEs, and license compatibility
- [/auth-engineering](../auth-engineering/SKILL.md) — authentication/authorization design reference (used by identity-security-developer)
- [/incident](../incident/SKILL.md) — guided incident response from detection through blameless postmortem
- [/run-quality-checks](../run-quality-checks/SKILL.md) — the full pre-commit quality gate, including SAST and SCA
